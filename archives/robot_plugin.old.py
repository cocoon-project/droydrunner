...
from droydrunner.robot_relay import DroydRelay
from droydrunner.uiconfig import UiConfiguration


#from syprunner.syprun.syprelay import SypRelay
...
class Pilot(object):
    ...
    def _call(self,user,address):
        """
            ...
            @sip_address
        """
        return self._execute(user,"phone.call_destination" , destination=address)
    ...
    def wait_incoming_call(self,user):
        ...
        #
        return self._execute(user,'phone.wait_incoming_call')
    ...
    def call_user(self,userA,userX,format="universal"):
        ...
        serial = agent.serial
        target_info = self.ptf.get_phone_config(serial)
    ...
    def answer_call(self,user,code=200):
        ...
        """
        print "user %s sends code %s" % ( user , str(code))
